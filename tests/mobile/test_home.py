...
class TestHome:
    ...
    def test_that_checks_learn_more_link(self, mozwebqa):
        ...
        assert "Add-ons are applications that let you personalize Firefox with extra functionality and style. Whether you mistype the name of a website or can't read a busy page, there's an add-on to improve your on-the-go browsing.",
    ...
    def test_all_featured_extensions_link(self, mozwebqa):
        ...
        home = Home(mozwebqa)
        assert home.is_the_current_page
        assert home.default_selected_tab_text, 'Featured'

        featured_extensions = home.click_all_featured_addons_link()

        assert featured_extensions.title == 'ADD-ONS'
        assert featured_extensions.page_header == 'Featured Extensions'
    ...
    def test_expandable_header(self, mozwebqa):
        ...
        assert menu_names == self.expected_menu_items
    ...
    def test_that_checks_the_tabs(self, mozwebqa):
        ...
        for tab in reversed(range(len(home.tabs[:-1]))):
            ...
            assert home.tabs[tab].is_tab_content_visible,
    ...
    def test_category_items(self, mozwebqa):
        ...
        for i in range(len(home.categories)):
            assert home.categories[i].name == self.expected_category_items[i]
